...
def setup_moviepy():
    try:
        ...
        try:
            ...
            print("ImageMagick found in PATH")
            # Even if found in PATH, we still want to set the explicit path
            magick_path = r"C:\Program Files\ImageMagick-7.1.1-Q16-HDRI\magick.exe"
            if os.path.exists(magick_path):
                print(f"Setting ImageMagick path to: {magick_path}")
                change_settings({"IMAGEMAGICK_BINARY": magick_path})
            return True
        except:
            print("ImageMagick not found in PATH, checking specific locations...")
        
        # Common ImageMagick installation paths
        possible_paths = [
            r"C:\Program Files\ImageMagick-7.1.1-Q16-HDRI\magick.exe",
            r"C:\Program Files\ImageMagick-7.1.1-46-Q16-HDRI\magick.exe",
        ...
        print("Could not find ImageMagick installation")
        return False
...
if not moviepy_configured:
    print("Warning: ImageMagick configuration failed. Video creation may not work properly.")

def verify_imagemagick():
    """Verify that ImageMagick is working correctly with MoviePy"""
    try:
        ...
        print("ImageMagick verification successful!")
        return True
    except Exception as e:
        print(f"ImageMagick verification failed: {e}")
        return False

# Verify ImageMagick configuration
imagemagick_verified = verify_imagemagick()
if not imagemagick_verified:
    print("Warning: ImageMagick verification failed. Please check your installation.")
...
def remove_vocals_with_spleeter(mp3_path):
    """Remove vocals using Spleeter for better quality"""
    try:
        ...
        update_status("Loading Spleeter model...")
        ...
        update_status("Separating audio (this may take a few minutes)...")
        
        # Separate the audio
        separator.separate_to_file(mp3_path, temp_dir)
        
        # Get the accompaniment path
        filename = os.path.splitext(os.path.basename(mp3_path))[0]
        accompaniment_path = os.path.join(temp_dir, filename, "accompaniment.wav")
        
        if not os.path.exists(accompaniment_path):
            return "Error: Separation failed", None
            
        # Convert to MP3 and clean up
        output_path = os.path.splitext(mp3_path)[0] + "_karaoke_spleeter.mp3"
        
        # Load and export with pydub for MP3 conversion
        audio = AudioSegment.from_wav(accompaniment_path)
...
def force_create_video():
    ...
    # Add progress label
    progress_label = ttk.Label(progress_window, text="Creating karaoke video...\nThis may take several minutes.")
    ...
    # Add cancel button
    cancel_button = ttk.Button(progress_window, text="Cancel", command=lambda: progress_window.destroy())
    cancel_button.pack(pady=10)
    
    def update_progress_status(text):
        try:
    ...
    def create_video_thread():
        try:
            ...
            update_progress_status("Creating video background...")
            ...
            update_progress_status("Processing lyrics for video...")
            ...
            update_progress_status("Compositing video...")
            ...
            # Write video file
            update_progress_status("Rendering karaoke video...")
            video.write_videofile(video_path, fps=fps, codec='libx264', 
                                 audio_codec='aac', threads=4)
            
            update_progress_status("Video created successfully!")
            messagebox.showinfo("Success", f"Karaoke video created: {video_path}")
